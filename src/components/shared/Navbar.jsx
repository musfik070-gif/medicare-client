...
export default function Navbar() {
  ...
  const getLinkClass = (path) => {
    ...
    return isActive
      ...
      : "text-slate-600 hover:text-sky-600 transition-all duration-200 rounded-none px-3 py-2.5 flex items-center h-full text-sm min-h-[44px] relative after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-sky-500 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300 after:origin-left";
  };

  const getMobileLinkClass = (path) => {
    const isActive = pathname === path;
    return isActive
      ? "block w-full text-sky-600 font-bold bg-sky-50 px-4 py-3 rounded-xl text-sm min-h-[44px]"
      : "block w-full text-slate-700 hover:text-sky-600 hover:bg-slate-50 transition-all px-4 py-3 rounded-xl text-sm min-h-[44px]";
  };

  return (
    <div className="sticky top-0 z-50 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100 font-sans">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4 min-h-[64px]">
        ...
        <button
          ...
          className="lg:hidden p-2.5 text-slate-600 hover:text-sky-500 rounded-xl hover:bg-slate-50 transition-colors focus:outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
        ...
        <ul className="hidden lg:flex menu menu-horizontal px-1 gap-2 items-center">
          ...
          {user?.role === "patient" && (
            <li>
              <Link href="/join-doctor" className="border border-sky-500 text-sky-500 hover:bg-sky-50/50 rounded-full px-4 py-2 font-medium transition-all duration-200 text-sm min-h-[44px] flex items-center">
                Join as a Doctor
              </Link>
            </li>
          )}
        ...
        {/* User Auth dropdown / action buttons */}
        <div className="flex items-center gap-3">
          {user ? (
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar ring-2 ring-sky-100 hover:ring-sky-300 transition-all duration-200 min-h-[44px] min-w-[44px]">
              ...
              >
                ...
                <li>
                  <Link href={user?.role ? `/dashboard/${user.role}` : "/dashboard"} className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-slate-600 hover:bg-sky-50 hover:text-sky-600 transition-all text-sm min-h-[44px]">
                    Dashboard
                  </Link>
                </li>
                <li className="mt-1">
                  <button type="button" onClick={handleLogout} className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-red-600 hover:bg-red-50 transition-all text-sm font-medium w-full text-left min-h-[44px]">
                    Logout
                  </button>
                </li>
              </ul>
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <Link href="/login" className="border border-sky-500 text-sky-500 hover:bg-sky-50 rounded-full px-4 md:px-5 py-2 font-medium transition-all duration-200 text-sm min-h-[44px] flex items-center">
                Login
              </Link>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-slate-100 shadow-inner py-4 animate-in slide-in-from-top-4 duration-200">
          <ul className="flex flex-col px-4 gap-2">
            ...
            {!user && (
              <li className="pt-2 border-t border-slate-100 mt-2 sm:hidden">
